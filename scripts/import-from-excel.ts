...
async function importPeople(wb: xlsx.WorkBook) {
  ...
  if (!ws) {
    console.log(`People: sheet "${SHEET}" not found.`);
    return;
  }
  const rows = sheetToRowsArray(ws);

  // Row 0 headers:
  // [Timestamp, What is your full name..., What Waros..., What's your phone number?]
  const HEADER_ROW = 0;
  const idxName = 1;
  ...
  for (let r = HEADER_ROW + 1; r < rows.length; r++) {
    ...
    const phone =
      phoneRaw !== "" && phoneRaw !== null && phoneRaw !== undefined
...
async function importSchedule(wb: xlsx.WorkBook) {
  ...
  const ws = wb.Sheets[SHEET];
  if (!ws) {
    console.log(`Schedule: sheet "${SHEET}" not found.`);
    return;
  }
  const rows = sheetToRowsArray(ws);

  // Dates are in row 2 (0-based), starting col 1
  const DATES_ROW = 2;
  const START_COL = 1;
  ...
  console.log(`Dates upserted (or already existed): ${upserts}`);
}
...
async function importShoeCounts(wb: xlsx.WorkBook) {
  ...
  const ws = wb.Sheets[SHEET];
  if (!ws) {
    console.log(`Shoes: sheet "${SHEET}" not found.`);
    return;
  }
  const rows = sheetToRowsArray(ws);

  // Row 0: ["Date", "Shoe Count", "Month", ...]
  const HEADER_ROW = 0;
  const idxDate = 0;
  const idxCount = 1;
  ...
  console.log(
    `ShoeCount updated: ${updated}; skipped rows: ${skipped}; dates without Gathering: ${missingGathering}`
  );
}

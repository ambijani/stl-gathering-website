...
export default function GatheringsPage() {
  ...
  async function onCreate(e: React.FormEvent) {
    ...
    const res = await fetch("/api/admin/gatherings", {
      ...
      body: JSON.stringify({ title: title || "Gathering", date: iso, notes: notes || "" })
    });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <form onSubmit={onCreate} className="border rounded p-4 grid gap-3 md:grid-cols-4">
        <input className="border p-2 rounded" placeholder="Title (optional)" value={title} onChange={(e) => setTitle(e.target.value)} />
        <input className="border p-2 rounded" type="date" value={date} onChange={(e) => setDate(e.target.value)} required />
        <input className="border p-2 rounded md:col-span-2" placeholder="Notes (optional)" value={notes} onChange={(e) => setNotes(e.target.value)} />
        <div className="md:col-span-4"><button className="px-4 py-2 rounded bg-black text-white">Create Date</button></div>
      </form>
              ...
              items.map((g) => {
                ...
                return (
                  <tr key={g._id} className="border-t">
                    ...
                    <td className="p-2">
                      <Link className="underline" href={`/admin/gatherings/${g._id}`}>Manage</Link>
                    </td>
                  </tr>
                );
              })
            ) : (
              <tr><td className="p-3 text-gray-500" colSpan={5}>No dates yet. Create one above.</td></tr>
            )}
          </tbody>
        </table>
      </div>

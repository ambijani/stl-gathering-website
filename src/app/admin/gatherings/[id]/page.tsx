...
type Varo = { _id: string; title: string; description?: string; location?: string; startTime?: string; endTime?: string; capacity?: number; tags?: string[]; assignedPeople?: string[]; };
...
export default function GatheringDetail() {
  ...
  const [vTitle, setVTitle] = useState(""); const [vCapacity, setVCapacity] = useState<number | undefined>(undefined);
  async function addVaro(e: React.FormEvent) {
    ...
    if (res.ok) { setVTitle(""); setVCapacity(undefined); await reloadVaros(); } else alert("Failed to add varo");
  }
  async function reloadVaros() {
    const r = await fetch(`/api/admin/gatherings/${id}/varos`);
    const varos = (await r.json()) as Varo[];
    setG((g0) => (g0 ? { ...g0, varos } : g0));
  }

  const [assignOpenFor, setAssignOpenFor] = useState<string | null>(null);
  const currentVaro = useMemo(() => g?.varos.find((v) => v._id === assignOpenFor) || null, [assignOpenFor, g]);
  const [selected, setSelected] = useState<Record<string, boolean>>({});
  function toggle(pid: string) { setSelected((s) => ({ ...s, [pid]: !s[pid] })); }
  async function assignSelected() {
    ...
    if (res.ok) { setSelected({}); await reloadVaros(); } else alert("Failed to assign");
  }
  ...
  async function deleteVaro(varoId: string) {
    ...
    if (res.ok) await reloadVaros(); else alert("Failed to delete");
  }
  ...
  async function saveShoeRows() {
    ...
    const res = await fetch(`/api/admin/gatherings/${id}/shoecount`, {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ items: cleaned })
    });
    if (!res.ok) alert("Failed to save shoe counts"); else alert("Shoe counts saved");
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">{g.title || "Date"} — {new Date(g.date).toLocaleDateString()}</h1>
      ...
      {tab === "varos" ? (
          ...
          <div className="border rounded">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr><th className="text-left p-2">Title</th><th className="text-left p-2">Capacity</th><th className="text-left p-2">Assigned</th><th className="text-left p-2">Actions</th></tr>
              </thead>
              <tbody>
                {g.varos?.map((v) => (
                  <tr key={v._id} className="border-t">
                    ...
                    <td className="p-2 flex gap-2">
                      <button className="underline" onClick={() => { setAssignOpenFor(v._id); setSelected({}); }}>Assign</button>
                      <button className="underline text-red-600" onClick={() => deleteVaro(v._id)}>Delete</button>
                    </td>
                  </tr>
                ))}
                {(!g.varos || !g.varos.length) && <tr><td className="p-4 text-gray-500" colSpan={4}>No Varos yet.</td></tr>}
              </tbody>
            </table>
          </div>
            ...
            <div className="p-4 border rounded">
              ...
              <div className="grid md:grid-cols-2 gap-4 mt-3">
                <div>
                  ...
                  <button className="mt-3 px-3 py-2 rounded bg-black text-white" onClick={assignSelected}>Assign Selected</button>
                </div>
                <div>
                  <h4 className="font-medium mb-2">Currently Assigned</h4>
                  <ul className="list-disc ml-5 text-sm">
                    {(currentVaro.assignedPeople || [])
                      .map((pid) => people.find((p) => p._id === pid)?.name || pid)
                      .map((name, i) => <li key={i}>{name}</li>)}
                    {(!currentVaro.assignedPeople || !currentVaro.assignedPeople.length) && <li className="text-gray-500">None yet.</li>}
                  </ul>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-3">
          <div className="border rounded">
            <table className="w-full text-sm">
              <thead className="bg-gray-50"><tr><th className="text-left p-2">Size</th><th className="text-left p-2">Qty</th><th className="text-left p-2">Remove</th></tr></thead>
              <tbody>
                {shoeRows.map((r, i) => (
                  <tr key={i} className="border-t">
                    <td className="p-2"><input className="border p-1 rounded w-28" value={r.size} onChange={(e) => updateRow(i, "size", e.target.value)} /></td>
                    ...
                    <td className="p-2"><button className="underline text-red-600" onClick={() => removeShoeRow(i)}>Remove</button></td>
                  </tr>
                ))}
                {!shoeRows.length && <tr><td className="p-3 text-gray-500" colSpan={3}>No rows. Add one below.</td></tr>}
              </tbody>
            </table>
          </div>
          <div className="flex gap-2">

...
export default function GatheringDetail() {
  ...
  async function addVaro(e: React.FormEvent) {
    ...
    if (res.ok) {
      setVTitle("");
      setVCapacity(undefined);
      await reloadVaros();
    } else {
      alert("Failed to add varo");
    }
  }
  ...
  async function assignSelected() {
    ...
    const res = await fetch(
      `/api/admin/gatherings/${id}/varos/${assignOpenFor}/assign`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function deleteVaro(varoId: string) {
    ...
    const res = await fetch(
      `/api/admin/gatherings/${id}/varos/${varoId}`,
      { method: "DELETE" }
    );
  ...
  useEffect(() => {
    if (g) setShoeRows(g.shoeCount || []);
  }, [g]);
  function updateRow(i: number, key: "size" | "qty", val: string) {
    setShoeRows((rows) =>
      rows.map((r, idx) =>
        idx === i ? { ...r, [key]: key === "qty" ? Number(val || 0) : val } : r
      )
    );
  }
  function addShoeRow() {
    setShoeRows((rows) => [...rows, { size: "", qty: 0 }]);
  }
  function removeShoeRow(i: number) {
    setShoeRows((rows) => rows.filter((_, idx) => idx !== i));
  }
  async function saveShoeRows() {
    const cleaned = shoeRows.filter(
      (r) => r.size && !Number.isNaN(r.qty)
    );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex gap-2">
        <button
          onClick={() => setTab("varos")}
          className={`px-3 py-1 rounded ${
            tab === "varos" ? "bg-black text-white" : "border"
          }`}
        >
          Varos
        </button>
        <button
          ...
          className={`px-3 py-1 rounded ${
            tab === "shoes" ? "bg-black text-white" : "border"
          }`}
        >
          Shoe Count
        </button>
      ...
      {tab === "varos" ? (
              ...
              <tbody>
                {g.varos?.map((v) => (
                  <tr key={v._id} className="border-t">
                    ...
                    <td className="p-2">
                      {typeof v.capacity === "number" ? v.capacity : "-"}
                    </td>
              ...
              <div className="grid md:grid-cols-2 gap-4 mt-3">
                  ...
                  <div className="max-h-72 overflow-auto border rounded">
                    <table className="w-full text-sm">
                      <tbody>
                        ...
                        {!people.length && (
                          <tr>
                            <td className="p-2">No people found.</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
                ...
                <div>
                  ...
                  <ul className="list-disc ml-5 text-sm">
                    ...
                    {(!currentVaro.assignedPeople ||
                      !currentVaro.assignedPeople.length) && (
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button className="px-3 py-2 rounded border" onClick={addShoeRow}>
              Add Row
            </button>
            <button
              className="px-3 py-2 rounded bg-black text-white"
              onClick={saveShoeRows}
            >
              Save
            </button>
          </div>

...
// POST: { personIds: string[], mode?: "merge" | "replace" }
export async function POST(
  ...
  if (mode === "replace") {
    v.assignedPeople = personIds;
  } else {
    for (const pid of personIds) current.add(String(pid));
    v.assignedPeople = Array.from(current);
  }
